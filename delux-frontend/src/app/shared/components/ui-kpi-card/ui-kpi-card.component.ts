  ...
  template: `
    <div class="card p-5">
      <div class="flex items-center justify-between">
        <p class="text-xs uppercase tracking-widest text-slate-500 dark:text-white/50 font-semibold">{{ label }}</p>
        @if (icon) {
          <div class="w-9 h-9 rounded-lg grid place-items-center"
               [ngClass]="iconBg ?? 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-white/70'">
            <i class="fa-solid {{ icon }} text-sm"></i>
          </div>
        }
      </div>
      <p class="mt-3 text-2xl md:text-3xl font-bold tracking-tight text-ink-950 dark:text-white">{{ value }}</p>
      @if (delta !== undefined) {
