...
def verify_recaptcha(token: str, remote_ip: str = '') -> bool:
    ...
    if not getattr(cfg, 'recaptcha_enabled', False):
        return True  # reCAPTCHA desactivado por el superadmin -> no se exige
    secret = (cfg.recaptcha_secret_key or '').strip()
    if not secret:
    ...
    try:
        ...
        # Si Google no responde, no bloqueamos el flujo.
        return True
